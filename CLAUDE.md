...
- `lib/betterAuth.ts` / `lib/authClient.ts` — **Better Auth** (email/password web login). It owns the `User`/`Session`/`Account`/`Verification` tables and is mounted at `app/api/auth/[...all]/route.ts`. `getServerSession()` reads the session in Server Components; `getSessionFromHeaders(headers)` reads it from a request in route handlers. It also runs its **own per-IP limiter on `/api/auth/**`** (separate from `lib/rateLimit.ts`, which exempts that prefix): limits are stated explicitly in `authRateLimit`, counters live in the `AuthRateLimit` table, and the client IP is resolved from `x-forwarded-for` past the proxies listed in the `TRUSTED_PROXIES` env var — leave that unset behind a multi-hop proxy and every caller shares one sign-in bucket.
...
- **Access log** (`lib/accessLog.ts`, ADR-0002): every non-exempt route leaves one `AccessLog` row per request, written after the response. Every handler is exported as `withAccessLog("<file's route pattern>", withRateLimit(…))` — log outermost so a 429 is recorded with its final status — and `tests/unit/routeCoverage.test.ts` fails a route missing either wrapper or whose pattern literal doesn't match its file path. Facts are recorded where discovered via `recordAccess(req, …)`: actor + credential in the auth resolvers, owner + resource ids in the owned loaders / `assertCanRead`, and the caller as owner in create/list handlers (`recordOwnAccount`). A null owner means no resource existed — never fall back to the actor, so record an owner only after a lookup that can 404 has succeeded.
