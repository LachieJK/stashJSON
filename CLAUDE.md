...
- **Access log** (`lib/accessLog.ts`, ADR-0002): every non-exempt route leaves one `AccessLog` row per request, written after the response. Every handler is exported as `withAccessLog("<file's route pattern>", withRateLimit(…))` — log outermost so a 429 is recorded with its final status — and `tests/unit/routeCoverage.test.ts` fails a route missing either wrapper or whose pattern literal doesn't match its file path. Facts are recorded where discovered via `recordAccess(req, …)`: actor + credential in the auth resolvers, owner + resource ids in the owned loaders / `assertCanRead`. A null owner means no resource existed — never fall back to the actor.
